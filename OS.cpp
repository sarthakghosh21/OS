...
void OS::MOS()
{
    if (SI == 1) // Read Mode
    {
        for (int i = 0; i <= 39; i++)
            buffer[i] = '\0';
        infile.getline(buffer, 40);// extracts value present in data card
        int k = 0;
        int i = IR[2] - 48;
        i = i * 10;
        for (int l = 0; l < 10; ++l)
...
void OS::Execute()
{
    while (true)
    {
        ...
        {
            IR[i] = M[IC][i]; // first line of the block is loaded -A
        }
        IC++;
        if (IR[0] == 'G' && IR[1] == 'D') // GD - Read
        {
            SI = 1;
            MOS();
        }
        ...
        {
            int i = IR[2] - 48;
            i = i * 10 + (IR[3] - 48); // extracting operand digits both ten's and units by subtracting the ASCII value of zero
            // cout<<i;
            for (int j = 0; j <= 3; j++)
                M[i][j] = R[j];
            cout << endl;
        }
        else if (IR[0] == 'C' && IR[1] == 'R') // CR - Compare Register
        {
            int i = IR[2] - 48;
            i = i * 10 + (IR[3] - 48);
            // cout<<i;
        ...
        else if (IR[0] == 'B' && IR[1] == 'T') // BT - Branch on True
        {
            if (C == true)
            {
                int i = IR[2] - 48;
...
void OS::LOAD()
{
    ...
    do
    {
        for (int i = 0; i <= 39; i++) // clear buffer
            buffer[i] = '\0';
        infile.getline(buffer, 40); // appends line by line
        for (int k = 0; k <= 39; k++)
            cout << buffer[k];
        ...
        else
        {
            ...
            for (; x < 100; ++x)
            {
                for (int j = 0; j < 4; ++j)
                {
                    M[x][j] = buffer[k]; // loading the buffer in memory
                    k++;
                }
